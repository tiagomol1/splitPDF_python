...
# variaveis
original_files = r"C:/automacao/original"
extract_to_boletos = r"C:/automacao/boletos"
extract_to_darf = r"C:/automacao/darf"
extract_to_nfe = r"C:/automacao/nfe"
extract_to_informes = r"C:/automacao/ajustados/informes"
...
def split_pdf_pages():
    ...
    for root, dirs, files in os.walk(original_files):
        ...
        for filename in files:
            directory = r"C:/automacao/ajustados"
            ...
            if extension == ".pdf":
                ...
                pdf_reader = pdf_file_obj.load_page(0)
                pdf_text = str(pdf_reader.get_text("text"))
                pdf_file_obj.close()
                ...
                if pdf_text.find("BRADESCO") != -1:
                    ...
                    directory = r"C:/automacao/arquivos incorretos"

                # abre o pdf no modo leitura
                opened_pdf = PyPDF2.PdfReader(open(fullpath, "rb"))
                print(opened_pdf.pages)
                print(len(opened_pdf.pages))
    ...
    for archive in archives:
        ...
        try:
            ...
            if pdf_text.find("Numero da NF-em") != -1 or pdf_text.find("Número da NF-em") != -1 :
                ...
                processDate = pdf_text.split('Data e Hora de Emissão')[1].split('\n')[1].replace('/', '-')[3:10]


                print('>>>>>COLENTANDO DADOS - fim')
                ...
                if pdf_text.find("Página 1/2") > -1:
                    ...
                    for archive2 in archives:
                        if pdf_text.find("Página 1/2") > -1:
                            ...
                            pdf_reader2 = pdf_file_obj2.load_page(0)
                            pdf_text2 = str(pdf_reader2.get_text("text"))
                            pdf_file_obj2.close()
                            ...
                            if pdf_text2.find("Página 2/2") == -1:
                                if nfeToCompare == nfe:
                                    ...
                                    time.sleep(1)
                                    x = x + 1
                                    merger.write('C:/automacao/nfe/teste'+ str(x) +'.pdf')
                                    merger.close()
                                    os.rename('C:/automacao/nfe/teste'+ str(x) +'.pdf', extract_to_nfe + "/" + clientName + " - NF " + nfe + " - (NFe )" + extension)
                                    data.append({
                                        "nfe": nfe,
                                        "processDate": processDate,
                                        "clientCnpj": clientCnpj,
