...
directory = r"R:/FINANCEIRO/_automacao/diretorioFinal"
def organizer(archivesData):
        ...
        if archive["processDate"] == '':
            ...
            for archiveCompare in archivesData:
                if len(archivesData) - 1 == x:
                    ...
                    os.rename(archive["archive"], "R:/FINANCEIRO/_automacao/ajustados/" + archive["clientName"] + " - NF " + archive["nfe"] + " - (" + archive["type"] + ")" + str(index) + ".pdf")
    ...
    for archive in archives:
        x = x + 1
        os.rename(archive, 'R:/FINANCEIRO/_automacao/ajustados/nf - '+ str(x) +'.pdf')